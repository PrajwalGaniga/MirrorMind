...
export default function Intelligence() {
  ...
  const playSilentFallback = async (text) => {
    stopSpeechRecognition();
    if (confTimeoutRef.current) clearTimeout(confTimeoutRef.current);
    try {
        ...
        if (handsFreeRef.current) {
          ...
          confTimeoutRef.current = setTimeout(() => {
            if (voiceStateRef.current === 'AWAITING_CONFIRMATION') {
              playSilentFallback("I'll wait. Say Hello MirrorMind whenever you need me.");
            }
          }, 8000);
        }
      };
  ...
  const handleConfirmation = (transcript) => {
    ...
    if (hasNo) {
      playSilentFallback("Okay. I'll wait for you.");
    } else if (hasYes) {
      stopSpeechRecognition();
      startRecording(true);
    } else {
      playSilentFallback("I didn't catch that. Please say yes when you're ready.");
    }
  };
  ...
  const handleMicClick = async () => {
    if (voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY') stopRecording();
    else if (voiceState === 'SPEAKING' || voiceState === 'GREETING' || voiceState === 'AWAITING_CONFIRMATION') {
      setHandsFree(false);
      stopPlayback();
    } else {
      setHandsFree(false);
  ...
  const startRecording = async (useSilenceDetection = false) => {
    try {
      setVoiceError('');
      setVoiceState(handsFreeRef.current ? 'LISTENING_FOR_QUERY' : 'LISTENING');
      setResult(null);
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      if (clean.length < 3 || invalidPhrases.includes(clean)) {
         if (handsFreeRef.current) {
            playSilentFallback("I didn't catch a question. Please try again.");
            return;
         } else {
  ...
  const synthesizeAndPlay = async (text) => {
    try {
      ...
      setVoiceError('Audio playback blocked by browser. Click once to enable.');
      setVoiceState('ERROR');
      if (handsFreeRef.current) {
        setTimeout(() => {
          if (handsFreeRef.current) {
      ...
      <form onSubmit={(e) => handleAsk(e, null)} style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'stretch' }}>
          <textarea
            ...
            disabled={loading || voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY' || voiceState === 'TRANSCRIBING'}
          />
          <button
            type="button"
            ...
            style={{
              ...
              backgroundColor: (voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY') ? '#ff4d4f' : voiceState === 'SPEAKING' ? '#52c41a' : undefined,
              color: (voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY' || voiceState === 'SPEAKING') ? '#fff' : undefined,
              border: (voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY' || voiceState === 'SPEAKING') ? 'none' : undefined,
              transition: 'background-color 0.3s',
            }}
            disabled={voiceState === 'TRANSCRIBING' || voiceState === 'THINKING' || loading}
            title={(voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY') ? 'Stop recording' : voiceState === 'SPEAKING' ? 'Stop speaking' : 'Use voice'}
          >
            {(voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY') ? <Square size={24} /> : voiceState === 'SPEAKING' ? <Volume2 size={24} /> : <Mic size={24} />}
          </button>
        </div>
        ...
        {voiceState !== 'IDLE' && (
          <div style={{ marginTop: 8, fontSize: 14, color: voiceState === 'ERROR' ? '#ff4d4f' : 'var(--primary-color)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {voiceState === 'WAKE_DETECTED' && <><span style={{ backgroundColor: '#52c41a', width: 8, height: 8, borderRadius: '50%', display: 'inline-block' }} /> ● MirrorMind heard you</>}
            {voiceState === 'GREETING' && <><Volume2 size={14} /> 🔊 MirrorMind is speaking...</>}
            ...
            {(voiceState === 'LISTENING_FOR_QUERY' || voiceState === 'LISTENING') && <><span style={{ backgroundColor: '#ff4d4f', width: 8, height: 8, borderRadius: '50%', display: 'inline-block' }} /> 🎤 Listening to your question...</>}
            ...
            {voiceState === 'SPEAKING' && <><Volume2 size={14} /> 🔊 Speaking...</>}
        ...
        <button
          ...
          disabled={loading || !question.trim() || voiceState === 'LISTENING' || voiceState === 'LISTENING_FOR_QUERY' || voiceState === 'TRANSCRIBING'}
