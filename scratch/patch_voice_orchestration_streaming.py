...
export default function Intelligence() {
  ...
  const controlRecognitionRef = useRef(null);
  ...
  useEffect(() => {
    if (handsFree) {
      ...
      streamingTTS.initAudio(); // Prompt user gesture context initialization
  ...
  // ── Stop/Continue Speech Controller ──
  const startControlListener = () => {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) return;
    
    stopSpeechRecognition();
    
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!controlRecognitionRef.current) {
      controlRecognitionRef.current = new SpeechRecognition();
      controlRecognitionRef.current.continuous = true;
      controlRecognitionRef.current.interimResults = false;
      controlRecognitionRef.current.lang = 'en-US';

      controlRecognitionRef.current.onresult = (event) => {
        const currentState = voiceStateRef.current;
        const last = event.results.length - 1;
        const transcript = event.results[last][0].transcript.trim().toLowerCase();
        const clean = transcript.replace(/[.,!?]/g, '');

        if ((currentState === 'SPEAKING' || currentState === 'SPEECH_PAUSED') && (clean === 'finish' || clean === 'cancel' || clean === 'stop speaking')) {
          console.log(`[MIRRORMIND][SPEECH_CONTROL] command="finish"`);
          stopControlListener();
          stopPlayback();
        ...
        } else if (currentState === 'SPEAKING' && (clean === 'stop' || clean === 'pause' || clean === 'stop mirrormind')) {
          console.log(`[MIRRORMIND][SPEECH_CONTROL] command="pause"`);
          streamingTTS.pause();
          setVoiceState('SPEECH_PAUSED');
          console.log(`[MIRRORMIND][SPEECH_CONTROL] speech_paused`);
        } else if (currentState === 'SPEECH_PAUSED' && (clean === 'continue' || clean === 'resume' || clean === 'continue mirrormind')) {
          console.log(`[MIRRORMIND][SPEECH_CONTROL] command="continue"`);
          streamingTTS.resume();
          setVoiceState('SPEAKING');
          console.log(`[MIRRORMIND][SPEECH_CONTROL] speech_resumed`);
        }
      };

      controlRecognitionRef.current.onend = () => {
        if (handsFreeRef.current && (voiceStateRef.current === 'SPEAKING' || voiceStateRef.current === 'SPEECH_PAUSED')) {
          try { controlRecognitionRef.current.start(); } catch (e) {}
        }
      };
    }
    
    try {
      console.log(`[MIRRORMIND][SPEECH_CONTROL] listener_started`);
      controlRecognitionRef.current.start();
    } catch (e) {}
  };

  const stopControlListener = () => {
    if (controlRecognitionRef.current) {
      controlRecognitionRef.current.onend = null;
      try { controlRecognitionRef.current.stop(); } catch (e) {}
      controlRecognitionRef.current = null;
      console.log(`[MIRRORMIND][SPEECH_CONTROL] listener_stopped`);
    }
  };

  // ── Generic TTS Player using WebSockets Streaming ──
  const playTTS = async (text, stateName, onEndedCallback, interruptible = false) => {
    stopSpeechRecognition();
    stopControlListener();
    setVoiceState(stateName);
    
    // We now use streamingTTS uniformly for BOTH hands-free and manual mode!
    if (interruptible && handsFreeRef.current) startControlListener();
    
    try {
      console.log(`[MIRRORMIND][AUDIO] Streaming TTS request started. text_length="${text.length}"`);
      await streamingTTS.speak(text);
      
      if (interruptible && handsFreeRef.current) stopControlListener();
      onEndedCallback();
    } catch (err) {
      if (interruptible && handsFreeRef.current) stopControlListener();
      console.error(`[MIRRORMIND][TTS][ERROR] streaming_failed:`, err);
      setVoiceError('Audio playback streaming failed.');
      setVoiceState('ERROR');
      setTimeout(() => onEndedCallback(), 2000);
    }
  };
  ...
  const handleAsk = async (e, customQuestion = null, overrideProvider = null) => {
    ...
    try {
      ...
      console.log(`[MIRRORMIND][VOICE] answer_chars=${llmAnswer ? llmAnswer.length : 0}`);
      console.log(`[MIRRORMIND][VOICE] answer_preview="${llmAnswer ? llmAnswer.substring(0, 150) : ''}"`);
      console.log(`[MIRRORMIND][TTS] source=LLM_ANSWER`);

      setResult(response.data); // Keep evidence in state for UI rendering
      setLoading(false); // Stop loading indicator immediately so UI displays answer
      stopStages();
      if (timerRef.current) clearInterval(timerRef.current);

      if (handsFreeRef.current || customQuestion) {
        // Pass ONLY the string answer to TTS, interruptible=true
        await playTTS(llmAnswer, 'SPEAKING', () => {
  ...
  const handleMicClick = async () => {
    ...
    else if (voiceState === 'SPEAKING' || voiceState === 'SPEECH_PAUSED' || voiceState === 'GREETING' || voiceState === 'CONFIRMING_TTS') {
      setHandsFree(false);
      cleanUpVoiceState();
    } else {
      setHandsFree(false);
        ...
        <div>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!handsFree) {
                // Pre-warm the audio context on user gesture
      ...
      <form onSubmit={(e) => handleAsk(e, null)} style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'stretch' }}>
          ...
          <button
            ...
            style={{
              ...
              backgroundColor: voiceState === 'LISTENING_FOR_QUERY' ? '#ff4d4f' : (voiceState === 'SPEAKING' || voiceState === 'SPEECH_PAUSED' || voiceState === 'GREETING' || voiceState === 'CONFIRMING_TTS') ? '#52c41a' : undefined,
              color: (voiceState === 'LISTENING_FOR_QUERY' || voiceState === 'SPEAKING' || voiceState === 'SPEECH_PAUSED' || voiceState === 'GREETING' || voiceState === 'CONFIRMING_TTS') ? '#fff' : undefined,
              border: (voiceState === 'LISTENING_FOR_QUERY' || voiceState === 'SPEAKING' || voiceState === 'SPEECH_PAUSED' || voiceState === 'GREETING' || voiceState === 'CONFIRMING_TTS') ? 'none' : undefined,
              transition: 'background-color 0.3s',
            }}
            disabled={voiceState === 'TRANSCRIBING' || voiceState === 'THINKING' || loading}
            title={voiceState === 'LISTENING_FOR_QUERY' ? 'Stop recording' : (voiceState === 'SPEAKING' || voiceState === 'SPEECH_PAUSED' || voiceState === 'GREETING' || voiceState === 'CONFIRMING_TTS') ? 'Stop speaking' : 'Use voice'}
          >
            {voiceState === 'LISTENING_FOR_QUERY' ? <Square size={24} /> : (voiceState === 'SPEAKING' || voiceState === 'SPEECH_PAUSED' || voiceState === 'GREETING' || voiceState === 'CONFIRMING_TTS') ? <Volume2 size={24} /> : <Mic size={24} />}
          </button>
        </div>
        ...
        {voiceState !== 'IDLE' && (
          <div style={{ marginTop: 8, fontSize: 14, color: voiceState === 'ERROR' ? '#ff4d4f' : 'var(--primary-color)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {voiceState === 'CONFIRMING_TTS' && <><Volume2 size={14} /> 🔊 Confirming your query...</>}
            {voiceState === 'AWAITING_CONFIRMATION' && <><span style={{ backgroundColor: '#fa8c16', width: 8, height: 8, borderRadius: '50%', display: 'inline-block' }} /> ● Waiting for confirmation...</>}
            ...
            {voiceState === 'SPEECH_PAUSED' && <><span style={{ color: '#fa8c16' }}>⏸ MirrorMind paused — say 'continue' to resume.</span></>}
