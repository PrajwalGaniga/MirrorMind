...
import { browserTTS, cleanTextForSpeech } from '../utils/browser-tts';
...
export default function Intelligence() {
  ...
  const controlRecognitionRef = useRef(null); // Dedicated stop/continue listener
  ...
  const startControlListener = () => {
    ...
    stopSpeechRecognition(); // Ensure main listener is off
    ...
    if (!controlRecognitionRef.current) {
      ...
      controlRecognitionRef.current.onresult = (event) => {
        ...
        if (currentState === 'SPEAKING' && (clean === 'stop' || clean === 'pause' || clean === 'stop mirrormind')) {
          console.log(`[MIRRORMIND][SPEECH_CONTROL] command="stop"`);
          browserTTS.pause();
  ...
  const playTTS = async (text, stateName, onEndedCallback, interruptible = false) => {
    ...
    if (handsFreeRef.current) {
      if (interruptible) startControlListener();
      try {
        await browserTTS.speak(text);
        if (interruptible) stopControlListener();
        onEndedCallback();
      } catch (err) {
        if (interruptible) stopControlListener();
        setVoiceError('Browser voice output failed.');
        setVoiceState('ERROR');
        console.error(`[MIRRORMIND][TTS][ERROR]`, err);
        setTimeout(() => onEndedCallback(), 2000);
      }
    } else {
      try {
        console.log(`[MIRRORMIND][AUDIO] TTS request started. text_length="${text.length}"`);
        ...
        console.error(`[MIRRORMIND][TTS][ERROR] playback_failed:`, err);
